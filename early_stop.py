...
def Stop_args_auroc(patience = 100, max_epochs=2000):
    return dict(stop_varnames=[StopVariable.AUROC, StopVariable.LOSS], patience=patience, max_epochs=max_epochs, remember=Best.RANKED)
    ...
    def __init__(
        ...
        for stop_varname in stop_varnames:
            if stop_varname is StopVariable.LOSS:
                ...
                self.best_vals.append(-np.inf)
            elif stop_varname is StopVariable.AUROC:
                self.stop_vars.append('auroc')
                self.comp_ops.append(operator.ge)
                self.best_vals.append(-np.inf)
